...
GLOBAL_Y_TOP    = 100   # rows above this are ignored (sky/ceiling)
GLOBAL_Y_BOTTOM = 290   # rows below this are ignored (robot body)
ROI_FAR_Y       = 190   # block here → flags set, avoidance steering begins
ROI_NEAR_Y      = 260   # block here + TFmini short → corner turn fires

# Orange/Blue floor line detection ROI
LINE_ROI  = (280, 200, 80, 40)   # (x, y, w, h)

# Close-block emergency dodge ROI (very thin strip — block is on top of us)
CLOSE_ROI = (250, 230, 140, 10)  # (x, y, w, h)
...
def _draw_overlay(canvas: np.ndarray, dets: dict, fps: float,
    ...
    """Draw ROI lines, bounding boxes, and HUD on the preview canvas."""
    W = canvas.shape[1]
    cv2.line(canvas, (0, ROI_FAR_Y),  (W, ROI_FAR_Y),  (255, 200, 0), 1)   # blue line
    cv2.line(canvas, (0, ROI_NEAR_Y), (W, ROI_NEAR_Y), (0,   0, 255), 1)   # red line
    ...
    cv2.putText(canvas,
                ...
                (6, 18), cv2.FONT_HERSHEY_SIMPLEX, 0.5, trig_col, 1)
    ...
    try:
        while True:
            ...
            if block_seen:
                ...
                # CONDITION A: block is deep in frame (very close to robot)
                roi_turn_trigger.value = (active_cy > ROI_NEAR_Y)
            else:
                if had_block:
                    no_block_count += 1
