...
def correctPosition(setPoint, head, x, y, trigger, counter, green, red):
	...
	if  not trigger:
		#print("In not trigger")
		getTFminiData()
		
		if setPoint == -70:
			print(f"Correcting Green Wall")
			if (distance_left  < 25 or (distance_head < 70 and distance_left> 100)):
				correction =   10 #20
		...
		if setPoint == 70:
			...
			if(distance_right < 25 or (distance_head < 70 and distance_right> 100)):
				correction =   -10
...
def servoDrive(pwm, color_b, stop_b, red_b, green_b, counts, centr_y):
		...
		try:
			...
			if button:
				...
				"""if(change_path):
					#correctAngle(0)
					counter = 0
					if(time.time() - red_time > 0.5):
						correctPosition(0, heading_angle - 90, x, y, flag_t, counter)
						correctPosition(0, heading_angle - 180, x, y, flag_t, counter)		
						
						
						
					print(f"x: {x}, y:{y}, turn_flag : {turn_f}, ")
					if(green_b.value and not red_b.value and not trigger and time.time() - red_time >= 0.2):

						green_time = time.time()
						
						correctPosition(-setPointL, heading_angle, x, y, flag_t, counter)

					elif(red_b.value and not green_b.value and not trigger and time.time() - green_time >= 0.2):

						red_time = time.time()
						correctPosition(-setPointR, heading_angle, x, y, flag_t, counter)	
					elif(not red_b.value and not green_b.value and not trigger and  time.time() - red_time >= 0.2 and  time.time() - green_time >= 0.2):

						correctPosition( 0, heading_angle, x, y, flag_t, counter)

					if(color_sensor == "Orange" and not trigger):
					...
					elif(color_sensor == "White" or color_sensor == "Blue"):
						trigger = False
						if(trig_counter >= 2):
							trig_counter = 0
	...
	#### for green path		
				...
				if reset_f:
					...
					if block_list[-1] == 'red':
						...
						if ((green_b.value or green_turn) and (color_sensor == "White")) and (distance_head > 35):
							...
							if not green_stored:
								green_stored = True
							green_path = time.time()
							correctPosition(0, heading_angle, x, y, trigger, counter, g_past, r_past)
						else:
					...
					if counter == 12 and not trigger:
						power = 0
						if centr_y.value > 400:
							if red_b.value and not stored_red and not green_b.value:
								block_list.append('red')
								stored_red = True
							if green_b.value and not stored_green and not red_b.value:
								block_list.append('green')
								stored_green = True
					...
					if green_b.value and not r_flag:
						...
						if green_stored:
							...
							green_stored = False
						print('5')
					# print(f"g: {g_past} r: {r_past}, red:{red_b.value}, green:{green_b.value}")
					# if(green_b.value and not trigger and time.time() - red_time >= 0.5):

					if g_flag:
						green_time = time.time()
						#print("Correcting green.....")
						correctPosition(setPointL, heading_angle, x, y, trigger, counter, g_past, r_past)

					# elif(red_b.value and not trigger and time.time() - green_time >= 0.5):
					elif r_flag:
						red_time = time.time()
						# print("correcting red...")
						green_stored = False
						correctPosition(
							setPointR, heading_angle, x, y, trigger, counter, g_past, r_past)

					# elif(not red_b.value and not green_b.value and not trigger and  time.time() - red_time >= 0.5 and  time.time() - green_time >= 0.5):
					else:
						# print("Correcting 0...")
						correctPosition(0, heading_angle, x, y, trigger, counter, g_past, r_past)

					if (color_sensor == 'Orange' and not trigger and (time.time() - turn_t) > (4 + buff)):
						buff = 0
						timer_started = False
						trigger = True
						reset_f = True
						...
						trigger = False
					#print(f"counter:{counts.value}")
					print(f"x:{x}, y:{y}, {glob} reset_f :{reset_f} counter:{counter} {trigger} {color_sensor} {imu.color_rgb}, green: {g_flag}, red: {r_flag}, green_stored: {green_stored}, g_past:{g_past}, r_past:{r_past}, left:{distance_left} right:{distance_right}, head:{distance_head}")
					#print(f"g_flag: {g_flag}, r_flag:{r_flag}, g_past:{g_past}, r_past:{r_past} trigger:{trigger}")
				...
				red_b.value = False
				green_b.value = False
			# orrectPosition(0, glob, x, y, flag_t, counter)
	...
	# print(f"distance_head: {distance_head}, left: {left}, right: {right}")
