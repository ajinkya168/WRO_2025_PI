...
def servoDrive(pwm, color_b, stop_b, red_b, green_b, pink_b, avoid_park, counts, centr_y,
               centr_y_red, centr_y_pink, g_next, r_next):
	...
	while True:
		...
		try:
			...
			if button:  ##### THIS BLOCK OF CODE WHEN BUTTON IS PRESSED
				...
				imu_head = glob 
				if change_path and not change_heading:
					reset_f = False
					trigger = False
					print(f"in else of change path...{i}")
	
					if i < 2:
						print(f"current {i} loop")
						offset = -90

						if abs(corr) < 10:
							heading_angle = heading_angle + offset						
							i += 1
							print(f"incrementing counter {i}")
					else:
						change_heading = True
						reset_flags = True
						print("Flag reset done..")
					'''if orange_flag and not change_heading:
						reset_f = False
						trigger = False
						heading_angle = heading_angle + offset
						...
						if i == 2:
							...
							print(f"offset is {offset} else")

							if abs(corr) < 5:
								offset = -90
								i = i + 1

					elif blue_flag and not change_heading:
						reset_f = False
						trigger = False
						heading_angle = heading_angle - offset
						...
						if i == 2:
							...
							if abs(corr) < 5 :
								offset = 90
								i = i + 1
							print(f"offset is {offset} else.. corr:{abs(corr)}.. {i}")'''
					change_counter = -1
				...
				if parking_flag and not stop_flag:
					...
					if (green_b.value and not r_flag) :
						...
						getTFminiData()
						if orange_flag:
							if distance_left <= 15 and (abs(prev_distance - distance_left) > 10) and p_past:
								p_past = False
								p_flag = False
								parking_flag = True
								print("Pink Avoidance Complete...")
						elif blue_flag:
							if distance_right <= 15 and (abs(prev_distance - distance_right) > 10) and p_past:
								p_past = False
								p_flag = False
								parking_flag = True
