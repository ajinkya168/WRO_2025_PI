...
def servoDrive(pwm, color_b, stop_b, red_b, green_b, pink_b, avoid_park, counts, centr_y):
	...
	while True:
		if pink_b.value:                ### DECIDES SETPOINT WHENEVER PINK IS IN THE FRAME
			setPointL = -10
			if counter == last_counter and not start_parking:
					g_flag = False
					g_past = False
					r_past = False
					green_stored = False
		...
		if start_parking:               ### THIS SETPOINT IS WHEN THE ROBOT IS IN THE PARKING MODE
			setPointL = -30
			...
			if button:              ##### THIS BLOCK OF CODE WHEN BUTTON IS PRESSED
				...
				if parking_flag and not stop_flag:
					...
					if not stop_flag and abs(corr) < 1:
						...
						correctAngle(heading_angle)
						getTFminiData()
						if (abs(corr) < 10) and (distance_head < 15 and distance_right < 10):
								power = 0
								prev_power = 0
								pwm.set_PWM_dutycycle(12, power)
								stop_flag = True
				...
				###### FOR
					...
					if reset_f:
						...
						if blue_flag:
							print("BLUE RESET...")
							if (red_b.value or r_past) and (distance_head > 20):
						...
						if orange_flag:
							...
							getTFminiData()
							x, y = enc.get_position(glob, counts.value)
							if ((g_flag or green_turn) and distance_head > 20):
								# if((green_b.value or green_turn) and centr_y.value < 700):
							...
							elif (p_past or p_turn) and distance_head > 20:
								p_turn = True
								green_turn = False
