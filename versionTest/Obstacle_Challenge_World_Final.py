                    ...
                    if STATE == 3:
                        if full_park:
                            ...
                            if blue_flag:
                                ...
                                final_park = time.time()
                                correctReverseAngle( heading_angle, head.value, 3)
                                while (parking_distance > 20) or abs(corr) > 5 and (time.time() - final_park < 1.9):
                                    tfmini.getTFminiData()
                                    if parking_right:
                                        parking_distance = tfmini.distance_right
                                    elif parking_left:
                        ...
                        if lap_finish:
                                ...
                                if time.time() - pink_time > pink_thresh:
                                    ...
                                    if orange_flag or blue_flag:
                                        ...
                                        if parking_right:
                                            ...
                                            if lidar_f.value < 1000 and lidar_r.value > 1200: 
                                                p_pass = 2
                                                if p_pass == 2:
                                                    p_past = False
                                                    parking_flag = True
                            ...
                            if OBSTACLE_STATE == 3:
                                ...
                                if orange_flag:
                                    ...
                                    correctWall(45, tfmini.distance_right, heading_angle, head.value, orange_flag, blue_flag, pink_wall_lane, counter)
                                    print("correcting middle wall")                                 #correctPosition(setPointC, heading_angle, x, y, counter, blue_flag, orange_flag, head.value, centr_x_pink.value, centr_x_red.value, centr_x.value, centr_y.value, centr_y_red.value, centr_y_pink.value, tf_h, tf_l, tf_r, red_b.value, green_b.value, pink_wall_lane )
