...
def servoDrive(pwm, color_b, stop_b, red_b, green_b, pink_b, avoid_park, counts, centr_y,
	...
	while True:
		...
		if lap_finish and pink_b.value:
			counter = counter % 12
			counter_reset = True
		...
		try:
			...
			if button:  ##### THIS BLOCK OF CODE WHEN BUTTON IS PRESSED
				...
				imu_head = glob + offset
				if change_path and time.time() - change_time < 3:
				...
				if parking_flag and not stop_flag:
					...
					if reset_f:
						...
						if blue_flag:  ### BLUE RESET BLOCK
							...
							if (red_b.value or red_turn) and distance_head > 20:  # red after trigger
								...
								power = 60
								heading_angle = -(((90 * counter) - offset) % 360)  
					...
					if (green_b.value and not r_flag) and not continue_parking:
						...
						p_flag = True
						getTFminiData()
						if distance_left <= 20 and (prev_distance - distance_left > 10) and p_past:
							p_past = False
							p_flag = False
							parking_flag = True
				...
				#print(f"x:{x}, y:{y}, heading:{heading_angle} {imu_head} color: {color_sensor}, orange:{orange_flag}, blue:{blue_flag}, corr:{corr}")
				#print(f"counter:{counter}, change_counter:{change_counter} g_count:{green_count}, r_count:{red_count}, change_path:{change_path}, turn_f:{turn_flag}")
