	...
	while True:
		...
		try:
			...
			if button:  ##### THIS BLOCK OF CODE WHEN BUTTON IS PRESSED
				...
				imu_head = glob
				if change_path:
					print(f"Change path is true..i = {i}, heading_angle {heading_angle}")
					offset = - 90
					if i <= 2:
						correctAngle(heading_angle)
						if abs(corr) < 5:
								...
								i += 1
								print(f"Taking first turn with heading_angle {heading_angle} and i = {i}")
					else:
						change_path = False
						print(f"Change Path is False. i = {i}")
