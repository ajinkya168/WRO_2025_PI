    ...
    try:
        while True:
            ...
            for obj in objs:
                ...
                '''x0, y0, x1, y1 = int(obj.bbox.xmin * scale_x), int(obj.bbox.ymin * scale_y), int(
                    obj.bbox.xmax * scale_x), int(obj.bbox.ymax * scale_y)
                cv2.rectangle(frame_bgr, (x0, y0), (x1, y1), (0, 255, 0), 2)
                cv2.putText(frame_bgr, f"{name} {obj.score:.2f}", (x0,
                            y0 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)'''
                # if area >= 2500:
    ...
    try:
        while True:
            ...
            if continue_parking:  # THIS SETPOINT IS WHEN THE ROBOT IS IN THE PARKING MODE
                ...
                g_flag = False
                r_flag = False
                # and (((centr_x_pink.value < centr_x.value) and (centr_x.value > 0 and centr_x_pink.value > 0)) or (centr_x_pink.value < centr_x_red.value and (centr_x_red.value > 0 and centr_x_pink.value > 0))):
                if orange_flag:
                    setPointR = -35
                    setPointC = -35
                    finish = True
            ...
            if button:  # THIS BLOCK OF CODE WHEN BUTTON IS PRESSED
                ...
                if parking_flag and not stop_flag:
                    ...
                    if reset_f:
                        ...
                        if orange_flag:
                            ...
                            while tfmini.distance_head < 90 and lidar_f.value < 850:
                                tfmini.getTFminiData()
                            ...
                            while lidar_f.value > 850 and tfmini.distance_head > 90:
                                print(f"correcting front distance forward {lidar_f.value}")
                                servo.setAngle(90)
                                tfmini.getTFminiData()
                                power = 70
                                prev_power = 0
                        ...
                        print(f"trigger_enc:{trigger_enc_flag} value:{trigger_enc} current:{counts.value}")
                        ...
                        elif red_b.value and not g_flag and not continue_parking and not r_flag and not reset_f and centr_y_red.value > 280:
                            r_flag = True
                            r_past = True
                        ...
                        print(
                            f"counts {counts.value} encoder: {encoder_counts_value} encoder updated: {encoder_counts_value + off} ")
                ...
                print(
                    f"trigger:{trigger} turn_trigger: {turn_trigger.value} reset_f:{reset_f} counter: {counter}, imu:{head.value}")
                print(
